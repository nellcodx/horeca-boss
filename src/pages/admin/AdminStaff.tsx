    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-heading font-bold text-2xl">Персонал</h2>
            <p className="text-muted-foreground text-sm">{staff.length} працівників</p>
          </div>
            ...
            <div key={member.id} className="p-5 rounded-xl bg-card border border-border flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-xs font-medium text-primary">{roleLabels[member.role].ua}</span>
                  <span className={cn('w-2 h-2 rounded-full', member.active ? 'bg-success' : 'bg-muted-foreground')} />
                  <span className="text-xs text-muted-foreground">{member.active ? 'Активний' : 'Неактивний'}</span>
                </div>
              </div>
